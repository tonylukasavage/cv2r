...
items.initItems = function initItems(romBuffer) {
  ...
  const whips = [
    ...
    { name: 'thorn whip', value: 0x01, icon: 0x5B, price: 100, bankCode: [] },
    { name: 'chain whip', value: 0x02, icon: 0x5B, price: 150, bankCode: [] },
    { name: 'morning star', value: 0x03, icon: 0x5B, price: 200, bankCode: [] },
    { name: 'flame whip', value: 0x04, icon: 0x5B, price: 226, bankCode: [] }
  ];
  bankIndexes.forEach(bankIndex => {
    const file = path.join(__dirname, 'asm', 'whip.asm');
    const loc = modSubroutine(romBuffer, file, bank[bankIndex]);
    whips.forEach(whip => {

...
weapons.forEach(w => {
  ...
  w.memory = 0x4A;
  w.code = Buffer.from([  
    0xA5, w.memory,    // LDA *$4A
    0x09, w.value,     // ORA ${w.value}
    0x85, w.memory     // STA *$4A
  ]);
});
...
inventory.forEach(i => {
  ...
  i.memory = 0x91;
  i.code = Buffer.from([  
    0xA5, i.memory,    // LDA *$91
    0x09, i.value,     // ORA ${w.value}
    0x85, i.memory     // STA *$91
  ]);
});
...
const itemizer = module.exports = async function itemizer(romBuffer, rng) {
  ...
  // create mapping of actors
  const mapping = [];
  itemActors.forEach(actor => {
    const { objset, area, submap } = actor;
    if (!mapping[objset]) { mapping[objset] = []; }
    if (!mapping[objset][area]) { mapping[objset][area] = []; }
    if (!mapping[objset][area][submap]) { mapping[objset][area][submap] = []; }
    mapping[objset][area][submap].push(actor.itemName); //.push(actor['7F']);
  });

  console.log(JSON.stringify(mapping, null, 2));

  // Dynamically write ROM function that will be responsible for determining what item
  // to give Simon based on the current actor's type id ($7F), objset ($30), area ($50), 
  // and submap ($51 AND #$0x0F)
  itemActors.forEach(actor => {

...
const { getActor, randomInt, textToBytes } = require('../utils');
...
const itemizer = module.exports = async function itemizer(romBuffer, rng) {
  ...
  // Write new text pointers for each individual merchant. Save the ROM location
  // of these pointers so that they can be referenced later when iterating 
  // through each actor.
  const textRam = {};
  const textValues = {};
  function addTextData(itemType) {
    const textRomLoc = bank[3].rom.start + bank[3].offset;
    const textRamLoc = bank[3].ram.start + bank[3].offset;
    const lowByte = textRamLoc & 0xFF;
    const highByte = textRamLoc >>> 8;
    textRam[itemType] = `
STA #$${lowByte}
STA #$${highByte}
`;
    textValues[itemType] = textRomLoc;
  }
  sharedItemTypes.forEach(addSaleData);

  // re-write code that determines if a merchant sells whips
  const isWhipCode = await fs.readFile(path.join(__dirname, 'asm', 'merchantId.asm'), 'utf8');
  const isWhipBuf = Buffer.from(assemble(isWhipCode));
  isWhipBuf.copy(romBuffer, bank[3].rom.start + bank[3].offset);
  const isWhipRam = bank[3].ram.start + bank[3].offset;
  const isWhipInvokeBuf = Buffer.from([ 0x20, isWhipRam & 0xFF, isWhipRam >>> 8, 0xEA ]);
  isWhipInvokeBuf.copy(romBuffer, 0x1EDAE);
  bank[3].offset += isWhipBuf.length;

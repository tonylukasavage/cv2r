...
const { getActor, modData, modSubroutine, pad, randomInt, textToBytes } = require('../utils');
...
const itemizer = module.exports = async function itemizer(romBuffer, rng) {
  ...
  itemActors.forEach(actor => {
    ...
    if ([ 'garlic', 'laurels' ].includes(item.name) || item.whip) {
...
`;
    ...
    console.log(`| ${pad(item.name, 13)} | ${pad(actor.name, 15)} | ${pad(actor.locationName, 37)} |`);
  });

  // new garlic/laurel/oak merchant text goes on bank 3
  file = path.join(__dirname, 'asm', 'textUnshare.asm');
  modSubroutine(romBuffer, file, bank[3], {
    exitOffset: 6,
    values: textValues,
    invoke: {
  ...
  file = path.join(__dirname, 'asm', 'boss.asm');
  modSubroutine(romBuffer, file, bank[1], {
    values: deathValues,
    invoke: {
      romLoc: 0x47D7,
  ...
  file = path.join(__dirname, 'asm', 'orb.asm');
  modSubroutine(romBuffer, file, bank[1], {
    values: orbValues,
    invoke: {
      romLoc: 0x47A7,
  ...
  file = path.join(__dirname, 'asm', 'merchantWhip.asm');
  modSubroutine(romBuffer, file, bank[3], {
  ...
  file = path.join(__dirname, 'asm', 'merchant.asm');
  modSubroutine(romBuffer, file, bank[3], {
    exitOffset: 4,

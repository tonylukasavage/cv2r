...
const { getActor, modSubroutine, randomInt, textToBytes } = require('../utils');
...
const itemizer = module.exports = async function itemizer(romBuffer, rng) {
  ...
  // write new pointers for copied data (+9 to skip garlic, laurel, oak entries)
  let origOffset = bank[3].rom.start + bank[3].offset;
  [
    null, // laurels,
    ...
    if (itemType) {
      getActor({ itemType }).salePointer = origOffset;
    }
    origOffset += 3;
  });

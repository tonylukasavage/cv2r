...
const itemizer = module.exports = async function itemizer(romBuffer, rng, opts = {}) {
  ...
  // reset SRAM
  modSubroutine(romBuffer, asmFile('sram.asm'), bank[7], {
    invoke: {
      romLoc: 0x1C02B,
      padding: 2
    }
  ...
  // This fixes the garlic merchant room, but also changes the night time palette
  ...
  // setup select -> up+a+b
  modSubroutine(romBuffer, asmFile('upAB.asm'), bank[7], {
    invoke: {
      romLoc: 0x1C09A
    }
  });

  // update starting x pos for select -> up+a+b
  modSubroutine(romBuffer, asmFile('upABpos.asm'), bank[8], {
    invoke: {
      romLoc: 0x1CFF8,
      padding: 1
    }
  ...
  // all of those anyway.
  Buffer.from([ 0x90, 0x3C ]).copy(romBuffer, 0x1EDB4);

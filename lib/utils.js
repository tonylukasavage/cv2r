...
//Copy of textToBytes using the textMapTitle table instead
//Would be better to make textToBytes take a second optional parameter to select the text table.
exports.textToBytesTitle = function textToBytesTitle(text) {
  return text.toUpperCase().split('').map(c => textMapTitle[c]).concat(END_BYTE);
}

//Changed this to use the modified textToBytesTitle function
exports.textToHexString = function(text) {
  const bytes = exports.textToBytesTitle(text);
  bytes.pop();

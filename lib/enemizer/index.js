...
const { enemiesBySpritePattern } = require('../../data/object');
...
module.exports = async function enemizer(romBuffer, rng) {
  ...
  core.forEach(loc => {
    ...
    const enemies = enemiesBySpritePattern[spritePattern];
    const len = loc.actors.length;
    loc.actors.forEach(a => {
      if (!a.enemy) { return; }
      a.typeId = enemies[randomInt(rng, 0, len - 1)];
    });
  });

...
class PatchManager {
	...
	getPatchData(ord, name) {
		...
		const patchPath = path.join(__dirname, '..', 'patch', ord, name);
		const patchJsonFile = path.join(patchPath, 'index.json');
		const patchIndexFile = path.join(patchPath, 'index.js');

		let hasIndex = true;
		let hasJson = true;
		try { fs.accessSync(patchIndexFile); } catch (err) { hasIndex = false; }
		try { fs.accessSync(patchJsonFile); } catch (err) { hasJson = false; }

		if (!hasIndex && hasJson) {
			return require(patchJsonFile);
		} else {

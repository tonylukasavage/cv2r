...
module.exports = async function create(rom, opts = {}) {
	...
	core.forEach(loc => {
		...
		Array.isArray(loc.actors) && loc.actors.forEach(a => {
			...
			if (a.item && a.itemPointer) {
				buf = Buffer.from(a.item);
				buf.copy(romBuffer, a.itemPointer);
			}

			// update choice, if necessary
			if (a.code && a.codePointer) {
				buf = Buffer.from(a.code);
				buf.copy(romBuffer, a.codePointer);
			}
		});

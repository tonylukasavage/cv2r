...
module.exports = async function create(rom, opts = {}) {
	...
	const titlePrint = (text, rom) => pm.core.add(textToBytes(text, TEXT_MAP_TITLE), rom);
  ...
  pm.core.add([ 0xB6 ], 0x80D0);

  // Fix Alba garlic room night time palette pointer
  pm.core.add([ 0x15 ], 0x1FA58);
  ...
  // pm.core.add([ 0x15, 0x2E ], 0x1FA5A);

  // reset SRAM
  modSubroutine('core', asmFile('sram.asm'), bank[8], {
    invoke: {
      romLoc: 0x1C02B,
      padding: 2
    }
  ...
  const clearObjectsLoc = modSubroutine('core', asmFile('clearObjects.asm'), bank[9]);

  // Allow de-selecting quest items
  const gameStateChecksLoc = modSubroutine('core', asmFile('gameStateChecks.asm'), bank[2]);
  
  // setup jovah warp and inventory deselect
  modSubroutine('core', asmFile('gameStateChecksWrapper.asm'), bank[7], {
    invoke: {
      romLoc: 0x1C09A
    },
    values: {
  ...
  modSubroutine('core', asmFile('jovahWarpPositioning.asm'), bank[8], {
    invoke: {
      romLoc: 0x1CFF8,
      padding: 1
    }
  ...
  modSubroutine('core', asmFile('jovahWarpFerryCheck.asm'), bank[8], {
    invoke: {
      romLoc: 0x1CF4C,
      padding: 1
    },
	...
	core.forEach(loc => {
		...
		Array.isArray(loc.actors) && loc.actors.forEach(a => {
			pm.core.add([ a.x, a.y, a.id, a.data ], a.pointer);
			...
			if (a.text && a.textPointer) {
				...
				for (let i = 0; i < textArray.length; i++) {
					pm.core.add(textToBytes(textArray[i]), textPointerArray[i]);
				}
			}
		});

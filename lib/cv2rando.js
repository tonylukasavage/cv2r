	...
	core.forEach(loc => {
		...
		Array.isArray(loc.actors) && loc.actors.forEach(a => {
			...
			if (a.text && a.textPointer) {
				buf = Buffer.from(textToBytes(a.text));
				buf.copy(romBuffer, a.textPointer);
			}

			// update choice, if necessary
			if (a.choice && a.choicePointer) {
				buf = Buffer.from(a.choice);
				buf.copy(romBuffer, a.choicePointer);
			}
		});

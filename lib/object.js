...
const npcs = [
  ...
  { 
    ...
    id: MERCHANT, 
    bank: 3,
    hack: function(romBuffer, item, bank, opts = {}) {
      // handle text for merchant
      this.text = `${item.name.replace(' ', '\n')}?`;
      
      // handle price and dialog
      // first byte is the icon, next 2 are the price (0x01 0x72 == 172)
      let price = Math.floor(item.price * (opts.priceMultiplier || 1));
      if (price > 256) { price = 256; }
      else if (price < 1) { price = 1; }
      this.sale = [ item.icon, Math.floor(item.price / 100), parseInt(item.price % 100, 16) ];

      // if garlic merchant
      // if (/garlic/.test(this.itemType)) {
      //   this.salePointer = bank.rom.start + bank.offset;
      //   this.salePointerRam = bank.ram.start + bank.offset;
      //   bank.offset += 3;
      // } 
      
      if (this.salePointer) {
        const buf = Buffer.from(this.sale);
        buf.copy(romBuffer, this.salePointer);
      }
    }
  },
...
npcs.forEach(({ name, id, hack, bank }) => {
  const key = camelcase(name);
